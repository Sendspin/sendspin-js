...
function updateStatusDisplay() {
  ...
  if (sync) {
    if (sync.clockDriftPercent !== undefined) {
      ...
      clockDrift.className = Math.abs(drift) > 0.1 ? 'status-value warning' : 'status-value';
    }

    if (sync.syncErrorMs !== undefined) {
      const error = sync.syncErrorMs;
      syncError.textContent = `${error.toFixed(1)}ms`;
      syncError.className = Math.abs(error) > 50 ? 'status-value warning' : 'status-value';
    }
    ...
    if (sync.resyncCount !== undefined) {
      ...
      resyncCount.className = sync.resyncCount > 5 ? 'status-value warning' : 'status-value';
    }
  }
}
