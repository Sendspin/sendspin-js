...
export class AudioScheduler {
  ...
  private currentCorrectionMethod: "none" | "samples" | "rate" | "resync" = "none";
  ...
  constructor(
    ...
    thresholdOverrides?: Partial<Record<CorrectionMode, Partial<CorrectionThresholds>>>,
  ) {
  ...
  private performGuardedCutover(
    ...
    this.recorrectionMonitor.minScheduleTimeSec = Math.max(cutoffTime, cutResult.keptTailEndTimeSec);
    this.nextPlaybackTime = 0;
    this.nextScheduleTime = 0;
    this.lastScheduledServerTime = 0;
    this.recorrectionMonitor.resetCheckState();
  ...
  private checkRecorrection(): void {
    if (!this.usesRecorrectionMonitor) { this.recorrectionMonitor.resetCheckState(); return; }
    if (!this.audioContext || this.audioContext.state !== "running") { this.recorrectionMonitor.resetCheckState(); return; }
    if (!this.stateManager.isPlaying || this.nextPlaybackTime === 0 || this.lastScheduledServerTime === 0) {
      this.recorrectionMonitor.resetCheckState(); return;
    }
    ...
    const outputLatencySec = this.useOutputLatencyCompensation ? this.latencyTracker.getSmoothedUs(this.audioContext) / 1_000_000 : 0;
    ...
    if (this.recorrectionMonitor.shouldRecorrect(Math.abs(smoothedSyncErrorMs), syncErrorMs, nowMs)) {
      this.performGuardedCutover("recorrection", { incrementResyncCount: true, markCooldown: true });
    }
  }

  getSyncDelayMs(): number { return this.syncDelayMs; }
  ...
  setSyncDelay(delayMs: number): void {
    ...
    if (this.scheduledSources.length === 0 && this.audioBufferQueue.length === 0 && this.nextPlaybackTime === 0) return;
    this.performGuardedCutover("delay-change", { incrementResyncCount: false, markCooldown: true });
  }

  get syncInfo(): {
    clockDriftPercent: number; syncErrorMs: number; resyncCount: number;
    outputLatencyMs: number; playbackRate: number;
    correctionMethod: "none" | "samples" | "rate" | "resync";
    samplesAdjusted: number; correctionMode: CorrectionMode;
  } {
    return {
  ...
  private emitStatusLog(nowMs: number): void {
    if (this._lastStatusLogMs !== 0 && nowMs - this._lastStatusLogMs < 10_000) return;
    this._lastStatusLogMs = nowMs;

    let corr: string;
    switch (this.currentCorrectionMethod) {
      case "rate": corr = `rate@${this.currentPlaybackRate}`; break;
      case "samples": corr = `samples:${this.lastSamplesAdjusted}`; break;
      default: corr = this.currentCorrectionMethod;
    }
    ...
    const latMs = this.latencyTracker.getSmoothedUs(this.audioContext) !== null
      ? Math.round(this.latencyTracker.getSmoothedUs(this.audioContext) / 1000) : 0;
  ...
  private resetSyncErrorEma(): void { this.smoothedSyncErrorMs = 0; }
  ...
  private adjustBufferSamples(buffer: AudioBuffer, samplesToAdjust: number): AudioBuffer {
    if (!this.audioContext || samplesToAdjust === 0 || buffer.length < 2) return this.copyBuffer(buffer);
  ...
  initAudioContext(): void {
    ...
    if ((navigator as any).audioSession) { (navigator as any).audioSession.type = "playback"; }
    ...
    if (this.outputMode === "direct") {
      ...
      if (!audioElement) throw new Error("Media-element output requires an audio element.");
      if (this.isAndroid && this.silentAudioSrc) {
        ...
        audioElement.play().catch((e) => { console.warn("Sendspin: Audio autoplay blocked:", e); });
      } else {
  ...
  async resumeAudioContext(): Promise<void> {
    if (this.audioContext && this.audioContext.state === "suspended") {
      try { await this.audioContext.resume(); console.log("Sendspin: AudioContext resumed"); }
      catch (e) { console.warn("Sendspin: Failed to resume AudioContext:", e); return; }
      if (this.audioBufferQueue.length > 0) this.scheduleQueueProcessing();
      if (this.usesRecorrectionMonitor) this.recorrectionMonitor.start();
    }
  }

  private cutScheduledSources(cutoffTime: number): { requeuedCount: number; cutCount: number; keptTailEndTimeSec: number } {
    if (!this.audioContext) return { requeuedCount: 0, cutCount: 0, keptTailEndTimeSec: 0 };
    ...
    this.scheduledSources = this.scheduledSources.filter((entry) => {
      if (entry.startTime < stopTime) { keptTailEndTimeSec = Math.max(keptTailEndTimeSec, entry.endTime); return true; }
      try { entry.source.onended = null; entry.source.stop(stopTime); } catch { /* ignore */ }
      this.audioBufferQueue.push({ buffer: entry.buffer, serverTime: entry.serverTime, generation: entry.generation });
      requeued++; cutCount++; return false;
    });
  ...
  updateVolume(): void {
    ...
    if (this.useHardwareVolume) { this.gainNode.gain.value = 1.0; return; }
  ...
  private scheduleQueueProcessing(): void {
    ...
    if (typeof globalThis.setTimeout === "function") {
      this.scheduleTimeout = globalThis.setTimeout(() => {
        this.scheduleTimeout = null; this.queueProcessScheduled = false; this.processAudioQueue();
      }, 15);
      return;
    }
    const run = () => { this.queueProcessScheduled = false; this.processAudioQueue(); };
    if (typeof (globalThis as unknown as { queueMicrotask?: unknown }).queueMicrotask === "function") {
      (globalThis as unknown as { queueMicrotask: (cb: () => void) => void }).queueMicrotask(run);
  ...
  handleDecodedChunk(chunk: DecodedAudioChunk): void {
    ...
    for (let ch = 0; ch < numChannels; ch++) audioBuffer.getChannelData(ch).set(chunk.samples[ch]);
    this.audioBufferQueue.push({ buffer: audioBuffer, serverTime: chunk.serverTimeUs, generation: chunk.generation });
    this.scheduleQueueProcessing();
  }
  ...
  processAudioQueue(): void {
    ...
    if (this.clockSource.pendingCutover) {
      ...
      if (this.scheduledSources.length > 0 || this.nextPlaybackTime !== 0 || this.lastScheduledServerTime !== 0) {
        this.performGuardedCutover("delay-change", { incrementResyncCount: false, markCooldown: false });
        return;
      }
    }
    ...
    while (this.audioBufferQueue.length > 0) {
      ...
      if (this.nextPlaybackTime > 0 && scheduledAheadSec >= targetScheduledHorizonSec) break;
      ...
      if (this.nextPlaybackTime === 0 || this.lastScheduledServerTime === 0) {
        ...
        if (this.recorrectionMonitor.minScheduleTimeSec !== null) {
          scheduleTime = Math.max(scheduleTime, this.recorrectionMonitor.minScheduleTimeSec);
        ...
        if (Math.abs(serverGapSec) < 0.1) {
          ...
          if (Math.abs(correctionErrorMs) > thresholds.resyncAboveMs && canHardResync) {
            ...
            scheduleTime = playbackTime - syncDelaySec;
            playbackRate = 1.0;
            this.currentCorrectionMethod = "resync"; this.lastSamplesAdjusted = 0;
            chunk.buffer = this.copyBuffer(chunk.buffer);
          } else if (Math.abs(correctionErrorMs) > thresholds.resyncAboveMs) {
            playbackTime = this.nextPlaybackTime; scheduleTime = this.nextScheduleTime;
            playbackRate = Number.isFinite(thresholds.rate2AboveMs) ? (correctionErrorMs > 0 ? 1.02 : 0.98) : 1.0;
            this.currentCorrectionMethod = playbackRate === 1.0 ? "none" : "rate"; this.lastSamplesAdjusted = 0;
            chunk.buffer = this.copyBuffer(chunk.buffer);
          } else if (Math.abs(correctionErrorMs) < thresholds.deadbandBelowMs) {
            playbackTime = this.nextPlaybackTime; scheduleTime = this.nextScheduleTime;
            playbackRate = 1.0; this.currentCorrectionMethod = "none"; this.lastSamplesAdjusted = 0;
            chunk.buffer = this.copyBuffer(chunk.buffer);
          } else if (Math.abs(correctionErrorMs) <= thresholds.samplesBelowMs) {
            playbackTime = this.nextPlaybackTime; scheduleTime = this.nextScheduleTime; playbackRate = 1.0;
            const samplesToAdjust = correctionErrorMs > 0 ? -1 : 1;
            chunk.buffer = this.adjustBufferSamples(chunk.buffer, samplesToAdjust);
            this.currentCorrectionMethod = "samples"; this.lastSamplesAdjusted = samplesToAdjust;
          } else {
            playbackTime = this.nextPlaybackTime; scheduleTime = this.nextScheduleTime;
            const absErrorMs = Math.abs(correctionErrorMs);
            if (correctionErrorMs > 0) {
              playbackRate = absErrorMs >= thresholds.rate2AboveMs ? 1.02 : absErrorMs >= thresholds.rate1AboveMs ? 1.01 : 1.0;
            } else {
              playbackRate = absErrorMs >= thresholds.rate2AboveMs ? 0.98 : absErrorMs >= thresholds.rate1AboveMs ? 0.99 : 1.0;
            }
            this.currentCorrectionMethod = playbackRate === 1.0 ? "none" : "rate"; this.lastSamplesAdjusted = 0;
            chunk.buffer = this.copyBuffer(chunk.buffer);
          }
      ...
      if (playbackTime < audioContextRawTimeSec) {
        this.nextPlaybackTime = 0; this.nextScheduleTime = 0; this.lastScheduledServerTime = 0; continue;
      }
      ...
      this.lastScheduledServerTime = chunk.serverTime + chunk.buffer.duration * 1_000_000;
  ...
  private computeTargetPlaybackTime(serverTimeUs: number, audioContextTime: number, nowUs: number, outputLatencySec: number): number {
  ...
  startAudioElement(): void {
    if (this.outputMode === "media-element" && this.audioElement?.paused) {
      this.audioElement.play().catch((e) => { console.warn("Sendspin: Failed to start audio element:", e); });
    }
  }
  ...
  clearBuffers(): void {
    ...
    if (this.scheduleTimeout !== null) { clearTimeout(this.scheduleTimeout); this.scheduleTimeout = null; }
  ...
  close(): void {
    ...
    if (this.outputMode === "media-element" && this.audioElement) {
      this.audioElement.pause(); this.audioElement.srcObject = null;
      this.audioElement.loop = false; this.audioElement.removeAttribute("src"); this.audioElement.load();
      if (this.ownsAudioElement) { this.audioElement.remove(); this.audioElement = undefined; }
    }
  }
